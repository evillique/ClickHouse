...
void CudaColumnString::addData(size_t str_num_, size_t str_buf_sz_, const char * str_buf_, const UInt64 * offsets_, cudaStream_t stream)
{
    CUDA_SAFE_CALL(cudaMemcpyAsync(getBuf() + sz, str_buf_, str_buf_sz_, cudaMemcpyHostToDevice, stream));
    // TODO do something with sizeof(UInt64)
    CUDA_SAFE_CALL(cudaMemcpyAsync(getOffsets64() + str_num, offsets_, str_num_ * sizeof(UInt64), cudaMemcpyHostToDevice, stream));
