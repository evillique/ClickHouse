...
BlockIO InterpreterRenameQuery::executeToTables(const ASTRenameQuery & rename, const RenameDescriptions & descriptions, TableGuards & ddl_guards)
{
    ...
    for (const auto & elem : descriptions)
    {
        ...
        std::vector<StorageID> from_hard_dependencies;
        std::vector<StorageID> from_mv_dependencies;
        std::vector<StorageID> to_ref_dependencies;
        std::vector<StorageID> to_hard_dependencies;
        ...
        if (exchange_tables)
        {
            ...
            std::tie(from_ref_dependencies, from_hard_dependencies, from_mv_dependencies) = database_catalog.removeDependencies(from_table_id, false, false, false, /*mv*/ true);
            std::tie(to_ref_dependencies, to_hard_dependencies, to_mv_dependencies) = database_catalog.removeDependencies(to_table_id, false, false, false, /*mv*/ true);
        }
        else
        {
            ...
            std::tie(from_ref_dependencies, from_hard_dependencies, from_mv_dependencies) = database_catalog.removeDependencies(from_table_id, check_ref_deps, check_hard_deps, false, /*mv*/ true);
        }
        try
        {
            ...
            DatabaseCatalog::instance().addDependencies(to_table_id, from_ref_dependencies, from_hard_dependencies, from_mv_dependencies);
            if (!to_ref_dependencies.empty() || !to_hard_dependencies.empty() || !to_mv_dependencies.empty())
                DatabaseCatalog::instance().addDependencies(from_table_id, to_ref_dependencies, to_hard_dependencies, to_mv_dependencies);
        }
        catch (...)
        {
            /// Restore dependencies if RENAME fails
            DatabaseCatalog::instance().addDependencies(from_table_id, from_ref_dependencies, from_hard_dependencies, from_mv_dependencies);
            if (!to_ref_dependencies.empty() || !to_hard_dependencies.empty() || !to_mv_dependencies.empty())
                DatabaseCatalog::instance().addDependencies(to_table_id, to_ref_dependencies, to_hard_dependencies, to_mv_dependencies);
            throw;
        }
    }
