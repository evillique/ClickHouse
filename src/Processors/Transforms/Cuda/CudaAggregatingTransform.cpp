...
public:
    CudaConvertingAggregatedToChunksTransform(CudaAggregatingTransformParamsPtr params_, CudaAggregatedDataVariantsPtr variants_, size_t /*num_threads_*/)
        ...
        , params(std::move(params_)), variants(std::move(variants_)) {}//, num_threads(num_threads_) {}
...
private:
    ...
    {
        // LOG_FATAL(&Poco::Logger::root(), "# {}:{} - setCurrentChunk(1)", __FILE__, __LINE__);
        if (has_input)
...
{
    // LOG_FATAL(&Poco::Logger::root(), "# {}:{} - {}::constructor(1)", __FILE__, __LINE__, "CudaAggregatingTransform");
}
